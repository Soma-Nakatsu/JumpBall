...
public class PlayerController : MonoBehaviour
{
    ...
    public void Run()
    {
        ...
        if (isDead)
        {
            ...
            // リスポーン位置にたどり着いていなければ
            // if (stageManager.TransTime == 0.0f)
            {
                // ゲーム状態をNONEへ
                stageManager.gameState = StageManager.GameState.NONE;
            }

            // nowTimeリスポーン位置通過時の時間に戻す
            //gameManager.NowTime = gameManager.TransTime;
        }
    }
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (other.gameObject.tag == "Wall" && isJump)
        {
            // 情報を保存
            wallJumpVec = new Vector3(x, jumpPower, z);
    ...
    {
        // ジャンプボタンが押されるかつジャンプ中じゃなければ
        if (Input.GetKey("joystick button 0") && !isJump)
        {
            // jumpPowerが最大でないときは
    ...
    /// <summary>
    /// プレイヤーは死んだか？のゲッター、セッター
    /// </summary>
    public bool SetIsDead { set { isDead = value; } }
